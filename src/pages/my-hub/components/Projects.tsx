...
const Projects: React.FC = () => {
  const projectsElements = PROJECTS.map((project) => (
    <div
      key={project.name}
      className="border-2 border-white/30 p-4 rounded-xl h-[210px] w-full max-w-[280px] flex flex-col"
    >
      <div
      ...
      <button
        ...
        className="main_btn outfit text-sm bg-white/80 text-black transition duration-300 hover:scale-105 ease-in-out w-fit mt-auto"
      >
        View Repo
      </button>
    </div>
  ...
  return (
    <section className="py-10 space-y-5 max-w-3xl w-full">
      <p className="font-bold fira p-1 px-3 w-fit bg-green-500/15 rounded-full text-rose-500/80 text-xs sm:text-lg z-10">
        Side Projects
      </p>
      <div className="flex flex-wrap items-center gap-8 justify-center text-green-500 bg-white/5 p-4">
        {projectsElements}
      </div>
    </section>
  );
