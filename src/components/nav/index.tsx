  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-4">
            ...
            {pathname !== "/projects" && (
              <Link
                to={"/projects"}
                className="main_btn pops bg-white text-black shadow-lg shadow-green-400/40 max-[375px]:hidden"
        ...
        <div
          className={`fixed left-1 top-0 w-[98vw] rounded-b-3xl bg-[#0e0f10] shadow-2xl h-[400px] z-50 md:hidden transform transition-transform ease-in-out duration-300 py-6 px-4 flex flex-col justify-between gap-4 shadow-emerald-400/10 ${
            navIsOpen ? "-translate-y-0" : "-translate-y-full"
          } shadow-2xl`}
        >
          <div className="absolute border w-full top-0 left-0 h-[250px] -z-10 bg-gradient-to-b blur-2xl from-rose-500/20 via-rose-500/20 to-transparent" />
          <div>
            <Logo />
          </div>
          <div className="flex flex-col gap-6 items-center outfit mt-4">
            <NavLink
              to={"/my-hub"}
              className={({ isActive }) =>
                `cursor-pointer duration-500 transition-all ${
          ...
          <X
            className="mx-auto mt-auto bg-white rounded-full p-2 text-black h-8 w-8"
            onClick={() => setNavIsOpen(false)}
          />
        </div>
      </div>
